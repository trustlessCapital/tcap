...
describe("GuardianHandler", function () {
    this.timeout(10000);
    ...
    let wallet, guardianStorage, guardianHandler;
    ...
    beforeEach(async () => {
        ...
        await wallet.init(owner.address, [guardianHandler.contractAddress]);
    });

    describe("Adding Guardians", () => {
        describe("EOA Guardians", () => {
            it("should let the owner add EOA Guardians (blockchain transaction)", async () => {
                await guardianHandler.from(owner).addGuardian(wallet.contractAddress, guardian1.address);
                let count = (await guardianStorage.guardianCount(wallet.contractAddress)).toNumber();
                let active = await guardianHandler.isGuardian(wallet.contractAddress, guardian1.address);
                assert.isTrue(active, 'first guardian should be active');
                assert.equal(count, 1, '1 guardian should be active');

                await guardianHandler.from(owner).addGuardian(wallet.contractAddress, guardian2.address);
                count = (await guardianStorage.guardianCount(wallet.contractAddress)).toNumber();
                active = await guardianHandler.isGuardian(wallet.contractAddress, guardian2.address);
                ...
                await guardianHandler.confirmGuardianAddition(wallet.contractAddress, guardian2.address);
                count = (await guardianStorage.guardianCount(wallet.contractAddress)).toNumber();
                active = await guardianHandler.isGuardian(wallet.contractAddress, guardian2.address);
            ...
            it("should not let the owner add EOA Guardians after two security periods (blockchain transaction)", async () => {
                ...
                await assert.revert(guardianHandler.confirmGuardianAddition(wallet.contractAddress, guardian2.address), "confirming the addition should throw");
            ...
            it("should only let the owner add an EOA guardian (blockchain transaction)", async () => {
                await assert.revert(guardianHandler.from(nonowner).addGuardian(wallet.contractAddress, guardian1.address), "adding from nonowner should throw");
            });

            it("should let the owner add an EOA guardian (relayed transaction)", async () => {
                await manager.relay(guardianHandler, 'addGuardian', [wallet.contractAddress, guardian1.address], wallet, [owner])
                const count = (await guardianStorage.guardianCount(wallet.contractAddress)).toNumber();
                const active = await guardianHandler.isGuardian(wallet.contractAddress, guardian1.address);
                assert.isTrue(active, 'first guardian should be active');
                assert.equal(count, 1, '1 guardian should be active');
            });
            ...
            it("should add many Guardians (blockchain transaction)", async () => {
                ...
                let count, active;
                for (let i = 1; i <= 5; i++) {
                    await guardianHandler.from(owner).addGuardian(wallet.contractAddress, guardians[i - 1].address);
                    if (i > 1) {
                        await manager.increaseTime(31);
                        await guardianHandler.confirmGuardianAddition(wallet.contractAddress, guardians[i - 1].address);
                    }
                    count = (await guardianStorage.guardianCount(wallet.contractAddress)).toNumber();
                    active = await guardianHandler.isGuardian(wallet.contractAddress, guardians[i - 1].address);
            ...
            it("should add many Guardians (relayed transaction)", async () => {
                ...
                let count, active;
                for (let i = 1; i <= 3; i++) {
                    await manager.relay(guardianHandler, 'addGuardian', [wallet.contractAddress, guardians[i - 1].address], wallet, [owner]);
                    if (i > 1) {
                        await manager.increaseTime(30);
                        await manager.relay(guardianHandler, 'confirmGuardianAddition', [wallet.contractAddress, guardians[i - 1].address], wallet, []);
                    }
                    count = (await guardianStorage.guardianCount(wallet.contractAddress)).toNumber();
                    active = await guardianHandler.isGuardian(wallet.contractAddress, guardians[i - 1].address);
                    assert.equal(count, i, 'guardian ' + i + ' should be added');
        ...
        describe("Smart Contract Guardians", () => {
            ...
            beforeEach(async () => {
                ...
                await guardianWallet1.init(guardian1.address, [guardianHandler.contractAddress]);
                guardianWallet2 = await deployer.deploy(Wallet);
                await guardianWallet2.init(guardian2.address, [guardianHandler.contractAddress]);
            ...
            it("should let the owner add Smart Contract Guardians (blockchain transaction)", async () => {
                await guardianHandler.from(owner).addGuardian(wallet.contractAddress, guardianWallet1.contractAddress);
                let count = (await guardianStorage.guardianCount(wallet.contractAddress)).toNumber();
                let active = await guardianHandler.isGuardian(wallet.contractAddress, guardian1.address);
                assert.isTrue(active, 'first guardian owner should be recognized as guardian');
                active = await guardianHandler.isGuardian(wallet.contractAddress, guardianWallet1.contractAddress);
                assert.isTrue(active, 'first guardian should be recognized as guardian');
                assert.equal(count, 1, '1 guardian should be active');

                await guardianHandler.from(owner).addGuardian(wallet.contractAddress, guardianWallet2.contractAddress);
                count = (await guardianStorage.guardianCount(wallet.contractAddress)).toNumber();
                active = await guardianHandler.isGuardian(wallet.contractAddress, guardian2.address);
                assert.isFalse(active, 'second guardian owner should not yet be active');
                active = await guardianHandler.isGuardian(wallet.contractAddress, guardianWallet2.contractAddress);
                assert.isFalse(active, 'second guardian should not yet be active');
                assert.equal(count, 1, 'second guardian should be pending during security period');

                await manager.increaseTime(30);
                await guardianHandler.confirmGuardianAddition(wallet.contractAddress, guardianWallet2.contractAddress);
                count = (await guardianStorage.guardianCount(wallet.contractAddress)).toNumber();
                active = await guardianHandler.isGuardian(wallet.contractAddress, guardian2.address);
            ...
            it("should let the owner add a Smart Contract guardian (relayed transaction)", async () => {
                await manager.relay(guardianHandler, 'addGuardian', [wallet.contractAddress, guardianWallet1.contractAddress], wallet, [owner])
                const count = (await guardianStorage.guardianCount(wallet.contractAddress)).toNumber();
                let active = await guardianHandler.isGuardian(wallet.contractAddress, guardianWallet1.contractAddress);
                assert.isTrue(active, 'first guardian should be active');
                active = await guardianHandler.isGuardian(wallet.contractAddress, guardian1.address);
            ...
            it("should not let owner add a Smart Contract guardian that does not have an owner manager", async () => {
                await assert.revert(guardianHandler.from(owner).addGuardian(wallet.contractAddress, dumbContract.contractAddress), "adding invalid guardian contract should throw");
            });
            ...
            describe("Non-Compliant Guardians", () => {
                ...
                it("it should fail to add a non-compliant guardian", async () => {
                    await assert.revert(guardianHandler.from(owner).addGuardian(wallet.contractAddress, nonCompliantGuardian.contractAddress, { gasLimit: 2000000 }));
                });
            });
        });
    });
    ...
    describe("Revoking Guardians", () => {
        ...
        it("should revoke a guardian (blockchain transaction)", async () => {
            await guardianHandler.from(owner).revokeGuardian(wallet.contractAddress, guardian1.address);
            let count = (await guardianStorage.guardianCount(wallet.contractAddress)).toNumber();
            let active = await guardianHandler.isGuardian(wallet.contractAddress, guardian1.address);
            ...
            await manager.increaseTime(30);
            await guardianHandler.confirmGuardianRevokation(wallet.contractAddress, guardian1.address);
            count = (await guardianStorage.guardianCount(wallet.contractAddress)).toNumber();
            active = await guardianHandler.isGuardian(wallet.contractAddress, guardian1.address);
        ...
        it("should not confirm a guardian revokation after two security periods (blockchain transaction)", async () => {
            ...
            await assert.revert(guardianHandler.confirmGuardianRevokation(wallet.contractAddress, guardian1.address), "confirming the revokation should throw");
        ...
        it("should add a guardian after a revoke (blockchain transaction)", async () => {
            ...
            await guardianHandler.from(owner).addGuardian(wallet.contractAddress, guardian3.address);
            await manager.increaseTime(30);
            await guardianHandler.confirmGuardianAddition(wallet.contractAddress, guardian3.address);
            count = (await guardianStorage.guardianCount(wallet.contractAddress)).toNumber();
    ...
    describe("Cancelling Pending Guardians", () => {
        ...
        it("owner should be able to cancel pending addition of guardian (blockchain transaction)", async () => {
            ...
            await guardianHandler.from(owner).addGuardian(wallet.contractAddress, guardian2.address);
            await guardianHandler.from(owner).cancelGuardianAddition(wallet.contractAddress, guardian2.address);
            await manager.increaseTime(30);
            await assert.revert(guardianHandler.confirmGuardianAddition(wallet.contractAddress, guardian2.address), "confirmGuardianAddition should throw");
        });
        ...
        it("owner should be able to cancel pending revokation of guardian (blockchain transaction)", async () => {
            ...
            await guardianHandler.from(owner).revokeGuardian(wallet.contractAddress, guardian1.address);
            await guardianHandler.from(owner).cancelGuardianRevokation(wallet.contractAddress, guardian1.address);
            await manager.increaseTime(30);
            await assert.revert(guardianHandler.confirmGuardianRevokation(wallet.contractAddress, guardian1.address), "confirmGuardianRevokation should throw");
        });

        it("owner should be able to cancel pending addition of guardian (relayed transaction)", async () => {
            // Add guardian 2 and cancel its addition
            await manager.relay(guardianHandler, 'addGuardian', [wallet.contractAddress, guardian2.address], wallet, [owner]);
            await manager.relay(guardianHandler, 'cancelGuardianAddition', [wallet.contractAddress, guardian2.address], wallet, [owner]);
            await manager.increaseTime(30);
            await assert.revert(guardianHandler.confirmGuardianAddition(wallet.contractAddress, guardian2.address), "confirmGuardianAddition should throw");
        });
        ...
        it("owner should be able to cancel pending revokation of guardian (relayed transaction)", async () => {
            // Revoke guardian 1 and cancel its revokation
            await manager.relay(guardianHandler, 'revokeGuardian', [wallet.contractAddress, guardian1.address], wallet, [owner]);
            await manager.relay(guardianHandler, 'cancelGuardianRevokation', [wallet.contractAddress, guardian1.address], wallet, [owner]);
            await manager.increaseTime(30);
            await assert.revert(guardianHandler.confirmGuardianRevokation(wallet.contractAddress, guardian1.address), "confirmGuardianRevokation should throw");
        });
    });
